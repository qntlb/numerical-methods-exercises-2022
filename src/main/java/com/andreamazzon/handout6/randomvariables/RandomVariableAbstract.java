...
public abstract class RandomVariableAbstract implements RandomVariableInterface {
	...
	public double generate() {
		/*
		 * (Generalized, see page 212 of the script) the script) inversion of the
		 * distribution function: here we use the fact (see page 215 of the script) that 
		 * X := F^(-1)(U) with U uniformly distributed in (0,1) and F^(-1) defined as
		 * F^(-1)(y) := inf{x|F(x) >= y} has cumulative distribution function F. 
		 * F^(-1) is here the quantile function of the random variable. The implementation of
